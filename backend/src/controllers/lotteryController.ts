...
interface DailySalesResult {
  ...
  source: 'snapshot' | 'live' | 'pos_fallback' | 'empty';
}
...
interface RealSalesFromSnapshotResult {
  totalSales: number;
  byBox: Map<string, BoxSale>;
}
 ...
 */
async function _realSalesFromSnapshots({
  orgId,
  storeId,
  dayStart,
  dayEnd,
}: RealSalesArgs): Promise<RealSalesFromSnapshotResult> {
  ...
  const boxMap = new Map<string, BoxSnapshot>(boxes.map((b) => [b.id, b]));
  ...
  function freshOpeningPosition(box: BoxSnapshot): string | null {
...
interface BestEffortArgs extends RealSalesArgs {
  isToday?: boolean;
}
 ...
 */
async function _bestEffortDailySales({
  orgId,
  storeId,
  dayStart,
  dayEnd,
  isToday = false,
}: BestEffortArgs): Promise<DailySalesResult> {
 ...
 */
async function _liveSalesFromCurrentTickets({
  orgId,
  storeId,
  dayStart,
}: LiveSalesArgs): Promise<RealSalesFromSnapshotResult> {
...
interface RealSalesRangeResult {
  totalSales: number;
  byDay: Array<{ date: string; sales: number; source: DailySalesResult['source'] }>;
  byGame: Map<string, GameSale>;
  source: 'snapshot' | 'live' | 'pos_fallback' | 'mixed' | 'empty';
}
 ...
 */
async function _realSalesRange({
  orgId,
  storeId,
  from,
  to,
}: RealSalesRangeArgs): Promise<RealSalesRangeResult> {
  ...
  const byDay: RealSalesRangeResult['byDay'] = [];
  const byGame = new Map<string, GameSale>(); // gameId → { sales, count }
  let totalSales = 0;
  ...
  const sourcesUsed = new Set<DailySalesResult['source']>();
  ...
  let source: RealSalesRangeResult['source'] = 'empty';
