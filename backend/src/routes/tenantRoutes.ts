...
interface PlanLimit { maxStores: number; maxUsers: number }
  ...
  try {
    ...
    await prisma.$transaction([
      ...
      prisma.userOrg.upsert({
        ...
        create: { userId, orgId: org.id, role: 'owner', isPrimary: true },
        update: { role: 'owner', isPrimary: true },
      }),
    ]);

    // Re-sync their default system role (staff → owner)
    await syncUserDefaultRole(userId).catch((e: unknown) =>
...
router.delete('/me', protect, requireTenant, authorize('superadmin', 'owner'), async (req, res, next) => {
  try {
    ...
    if (errCode(err) === 'P2025') return res.status(404).json({ error: 'Organisation not found.' });
    next(err);
  }
});
