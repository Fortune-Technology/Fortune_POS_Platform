...
// ── Public types (used by callers + tests) ───────────────────────────────
...
function axiosErrMessage(err: unknown): string {
...
function resolveBaseUrl(merchant: DejavooHppMerchant): string {
...
function resolveQueryStatusBaseUrl(merchant: DejavooHppMerchant): string {
  const env: 'uat' | 'prod' = merchant.environment === 'prod' ? 'prod' : 'uat';
 ...
 *   $1.50   → "150"
 */
function toCents(dollars: unknown): string | undefined {
