...
import { awardBirthdayBonus, expireCustomerPoints } from './loyaltyService.js';
  ...
  try {
    ...
    for (const c of candidates) {
      try {
        ...
        console.error(`[LoyaltyScheduler] birthday error for customer ${c.id}:`, err.message);
      }
    }
    ...
    for (const p of programs) {
      ...
      for (const c of customers) {
        try {
          ...
          console.error(`[LoyaltyScheduler] expiry error for customer ${c.id}:`, err.message);
        }
      }
    }
    ...
    console.error('[LoyaltyScheduler] fatal sweep error:', err.message);
  } finally {
    running = false;
  }
}

export function startLoyaltyScheduler() {
  if (timer) return;
...
export function stopLoyaltyScheduler() {
