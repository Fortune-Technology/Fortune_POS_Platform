 ...
 */
export async function computePointsEarned({ storeId, lineItems }) {
  ...
  for (const li of items) {
    if (li.isLottery || li.isFuel || li.isBottleReturn || li.isBagFee || li.qty <= 0) continue;
 ...
 */
async function _writeHistory({ orgId, customerId, delta, reason, txId, txNumber, meta }) {
  ...
  await prisma.customer.update({
    where: { id: customerId },
    data:  { loyaltyPoints: newBal, pointsHistory: [...history, entry] },
  });
...
export async function processTransactionPoints({
  ...
  const redeemed = Math.max(0, parseInt(loyaltyPointsRedeemed) || 0);
 ...
 */
export async function reverseTransactionPoints({ originalTx, reason = 'void_reverse' }) {
  if (!originalTx) return;
  const { orgId, storeId, lineItems, id: txId, txNumber } = originalTx;
 ...
 */
export async function awardWelcomeBonus({ orgId, customerId, storeId }) {
  if (!storeId) return 0;
  const program = await prisma.loyaltyProgram.findUnique({ where: { storeId } });
 ...
 */
export async function awardBirthdayBonus({ orgId, customer }) {
 ...
 */
export async function expireCustomerPoints({ orgId, customer, expiryDays }) {
