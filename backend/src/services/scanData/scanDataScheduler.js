/**
 * scanDataScheduler.js — Nightly tobacco scan-data submission cron (Session 47).
 *
...
const ALREADY_SUBMITTED_WINDOW_HOURS = 23; // don't double-submit within 23h
...
async function shouldSubmit({ enrollment, store }) {
...
async function tick() {
  try {
    ...
    const storeIds = [...new Set(enrollments.map(e => e.storeId))];
    ...
    for (const e of enrollments) {
      ...
      try {
        ...
        console.error(`[ScanDataScheduler] ${e.manufacturer.code} for ${store.name}:`, err.message);
        failed++;
      }
    }
    ...
    console.error('[ScanDataScheduler] tick failed:', err.message);
  }
}

let _started = false;

export function startScanDataScheduler() {
  if (_started) return;
  _started = true;
  ...
  setInterval(() => { tick().catch(() => {}); }, SWEEP_INTERVAL_MS);
}
