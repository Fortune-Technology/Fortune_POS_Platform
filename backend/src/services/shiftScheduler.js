...
async function autoCloseShift(shift) {
...
export async function runShiftSweep() {
  ...
  const tzByStoreId = Object.fromEntries(stores.map(s => [s.id, s.timezone || 'UTC']));
  ...
  for (const shift of openShifts) {
    try {
      ...
      console.error(`[ShiftScheduler] Failed to auto-close shift ${shift.id}: ${err.message}`);
    }
  }
  if (closed > 0) {
...
export function startShiftScheduler() {
  ...
  setTimeout(() => { runShiftSweep().catch(err => console.error('[ShiftScheduler] sweep error:', err.message)); }, 30 * 1000);
  setInterval(() => { runShiftSweep().catch(err => console.error('[ShiftScheduler] sweep error:', err.message)); }, SWEEP_INTERVAL_MS);
}
