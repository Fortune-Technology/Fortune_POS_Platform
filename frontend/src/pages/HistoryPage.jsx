            ...
            {history.length === 0 ? (
                <div className="card p-xl text-center">
                    <div className="card-body">
                        ...
                        <p className="text-secondary mb-xl">Upload a file to get started with the CSV transformer.</p>
                        <button className="btn btn-primary" onClick={() => navigate('/csv/upload')}>Upload File</button>
                    </div>
                </div>
            ) : (
                <div className="card">
                    <div className="table-container">
                        <table className="table">
                            <thead>
                                <tr>
                                    <th>Filename</th><th>Status</th><th>Rows</th><th>Warnings</th><th>Created</th><th>Completed</th><th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {history.map((item) => (
                                    <tr key={item.transformId}>
                                        <td>
                                            ...
                                            <span className="hp-transform-id">{item.transformId.substring(0, 8)}...</span>
                                        </td>
                                        <td>{getStatusBadge(item.status)}</td>
                                        <td>{item.rowsProcessed?.toLocaleString() || '-'}</td>
                                        <td>
                                            {item.warningCount > 0 ? (
                                                <span className="badge badge-warning">{item.warningCount}</span>
                                            ) : (
                                        ...
                                        <td>
                                            <div className="hp-actions">
                                                {item.status === 'completed' && (
                                                    <button className="btn btn-sm btn-primary" onClick={() => handleDownload(item.transformId)}>⬇️ Download</button>
                                                )}
                                                <button className="btn btn-sm btn-secondary" onClick={() => navigate(`/transform/${item.transformId}`)}>👁️ View</button>
                                                <button className="btn btn-sm btn-error" onClick={() => handleDelete(item.transformId)} disabled={deleting === item.transformId}>
