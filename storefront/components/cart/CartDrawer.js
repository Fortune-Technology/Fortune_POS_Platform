...
export default function CartDrawer() {
  const { items, cartTotal, drawerOpen, setDrawerOpen, updateQty, removeItem, storeSlug } = useCart();
    ...
    <>
      ...
      <div className={`cd-drawer ${drawerOpen ? 'cd-drawer--open' : ''}`}>
        <div className="cd-header">
          ...
          <button className="cd-close" onClick={() => setDrawerOpen(false)}>&times;</button>
        </div>
              ...
              <div key={item.productId} className="cd-item">
                ...
                <div className="cd-item-info">
                  ...
                  <div className="cd-item-controls">
                    ...
                    <button className="cd-qty-btn" onClick={() => updateQty(item.productId, item.qty + 1)}>+</button>
                    <button className="cd-remove" onClick={() => removeItem(item.productId)}>Remove</button>
                  </div>
                </div>
              </div>
            ))
        ...
        {items.length > 0 && (
          <div className="cd-footer">
            ...
            <div className="cd-actions">
              <Link href={`/checkout?store=${storeSlug}`} className="cd-btn-checkout" onClick={() => setDrawerOpen(false)}>
                Checkout
              </Link>
              <Link href={`/cart?store=${storeSlug}`} className="cd-btn-viewcart" onClick={() => setDrawerOpen(false)}>
                View Full Cart
              </Link>
            </div>
          </div>
